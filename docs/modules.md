...
| `app/src/App.tsx` | Define `<HashRouter>`, renderiza `<Sidebar/>`, `<CommandPalette/>` e `<ToolRoutes/>`. `ToolRoutes` envolve as 18 `<Route>` num `<ErrorBoundary resetKey={location.pathname}>` + `<Suspense fallback={<RouteFallback/>}>`. Cada componente-ferramenta é `React.lazy(() => import(...))` — code-split por rota, chunk carregado sob demanda. |
...
| `app/src/components/ToolLayout.tsx` | Shell padrão de uma ferramenta: `<div className="main-content">` + `.tool-header` (título/descrição). `children` fica livre para ter 1+ `.tool-body` — necessário pro `JwtDecoderTool`, que tem uma barra de abas entre o header e dois `.tool-body` condicionais. Usado pelas 18 tools. |
...
| `ImageBase64Tool` | `/image-base64` | — (`FileReader`) | Abas **Codificar** (`<input type="file" accept="image/*">` → `FileReader.readAsDataURL`; preview, nome/tipo/tamanho do arquivo, checkbox pra incluir/remover o prefixo `data:URI`) e **Decodificar** (cola Base64 puro ou data URI — se faltar o prefixo, usa o `<select>` de tipo MIME para montá-lo; valida com `atob`; preview + tamanho estimado calculado a partir do comprimento do Base64 + padding; botão "Baixar" via `<a download>` com o data URI direto). | encode: `file`, `dataUrl`, `includePrefix`, `encodeError`; decode: `decodeInput`, `mimeType` (+ `decodedSrc`/`decodeError`/`decodedSize` via `useMemo`); `mode` |
